...
async function supabaseUserFetch(path, authorization) {
  const response = await fetch(`${SUPABASE_URL}${path}`, { headers: { apikey: SUPABASE_ANON_KEY, Authorization: authorization, "Content-Type": "application/json" } });
...
export default async function handler(req, res) {
  ...
  if (!SUPABASE_URL || !SUPABASE_ANON_KEY || !hasSupabaseServerConfig() || !PAYSTACK_SECRET_KEY) return json(res, 503, { error: "Premium payment provider is not configured" });
  ...
  try {
    ...
    const paymentResult = await supabaseServerFetch(`/rest/v1/premium_payments?id=eq.${encodeURIComponent(paymentId)}&select=id,user_id,plan_id,amount,currency,status,provider_reference&limit=1`);
    ...
    const userResult = await supabaseUserFetch("/auth/v1/user", authorization);
    ...
    const activation = await supabaseServerRpc("activate_premium_payment", { p_payment_id: payment.id, p_provider_reference: normalizedReference, p_paid_amount: payment.amount, p_paid_currency: payment.currency });
    return json(res, 200, { verified: true, activation, providerReference: normalizedReference });
  } catch (error) {
