...
const effectiveRoleCodes = (account) => (Array.isArray(account?.effectiveRoles) && account.effectiveRoles.length ? account.effectiveRoles : (account?.roles || []).map((role) => typeof role === "string" ? role : role.code).filter(Boolean));
...
function ArtistWorkspace({ nav, account }) {
  ...
  const saveSong = async (event) => {
    ...
    try { const cover = songCoverFile ? await uploadMediaFile(account.user.id, songCoverFile, "MUSIC_COVER", "songs", editingSong.id) : null; const audio = songAudioFile ? await uploadMediaFile(account.user.id, songAudioFile, "AUDIO", "songs", editingSong.id) : null; const updated = await updateArtistSong(editingSong.id, artist.id, { ...editingSong, cover_url: cover?.public_url || editingSong.cover_url, audio_url: audio?.public_url || editingSong.audio_url }); setWorkspace((current) => ({ ...current, songs: current.songs.map((song) => song.id === updated.id ? updated : song) })); setSongCoverFile(null); setSongAudioFile(null); setEditingSong(null); setMessage("Song details saved."); } catch (saveError) { setError(saveError.message || "Unable to save song details."); } finally { setBusy(false); }
  };
...
function GovernanceDashboard({ nav, account }) {
  ...
  const refresh = async () => { setBusy(true); try { const [data, eventRows] = await Promise.all([loadRoleGovernanceSnapshot(), loadGovernanceEvents()]); setSnapshot(data); setEvents(eventRows); setFee(data.fees?.[0] || null); } catch (error) { setNotice(error.message || "Unable to load governance data."); } finally { setBusy(false); } };
  useEffect(() => { refresh(); }, []);
...
function VenueManagerWorkspace({ nav, account }) {
  ...
  const create = async (event) => { event.preventDefault(); setBusy(true); setError(""); try { const photo = venuePhotoFile ? await uploadMediaFile(account.id, venuePhotoFile, "VENUE_PHOTO", "venues", null) : null; await createOwnedVenue(account.id, { ...form, capacity: Number(form.capacity), pricing: form.pricing ? { base: Number(form.pricing) } : {}, image_urls: photo?.public_url ? [photo.public_url] : [] }); setVenuePhotoFile(null); setForm(blankVenue); await load(); } catch (err) { setError(err.message || "Unable to create venue."); } finally { setBusy(false); } };
  ...
  return (
    <Phone>
      ...
      <div className="flex-1 overflow-y-auto px-5">
        ...
        {!hasEffectiveRole(account, "ARTIST") && <button onClick={() => nav.push("artistOnboarding")} className="w-full flex items-center justify-between py-3.5" style={{ borderBottom: `1px solid ${C.line}` }}><span className="text-[13.5px]" style={{ color: C.goldSoft }}>Become an Artist</span><ChevronRight size={15} color={C.muted} /></button>}
        {effectiveRoleCodes(account).some((role) => ["VENUE_MANAGER", "ADMIN", "SUPER_ADMIN"].includes(role)) ? <button onClick={() => nav.push("venueManager")} className="w-full flex items-center justify-between py-3.5" style={{ borderBottom: `1px solid ${C.line}` }}><span className="text-[13.5px]" style={{ color: C.goldSoft }}>Venue Manager Workspace</span><ChevronRight size={15} color={C.muted} /></button> : <button onClick={() => nav.push("venueOnboarding")} className="w-full flex items-center justify-between py-3.5" style={{ borderBottom: `1px solid ${C.line}` }}><span className="text-[13.5px]" style={{ color: C.goldSoft }}>Become a Venue Manager</span><ChevronRight size={15} color={C.muted} /></button>}
        ...
        {hasEffectiveRole(account, "ARTIST") && <button onClick={() => nav.push("artistVerification")} className="w-full flex items-center justify-between py-3.5" style={{ borderBottom: `1px solid ${C.line}` }}><span className="text-[13.5px]" style={{ color: C.goldSoft }}>Get Verified</span><ShieldCheck size={15} color={C.gold} /></button>}
        {items.map((it) => (
