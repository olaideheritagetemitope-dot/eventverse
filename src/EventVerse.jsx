...
function PlaylistDetail({ nav, account, player, catalog, data }) {
  ...
  const save = async (nextVisibility = visibility) => { if (!account?.user?.id || !playlist?.id) return; setSaving(true); try { const updated = await updatePlaylist(account.user.id, playlist.id, { name, description, visibility: nextVisibility }); setPlaylist(updated); setVisibility(updated.visibility || nextVisibility); setMessage(updated.visibility === "PUBLIC" ? "Playlist published publicly." : "Playlist saved privately."); } catch (error) { setMessage(error.message || "Unable to update playlist."); } finally { setSaving(false); } };
