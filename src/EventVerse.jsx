...
function ArtistWorkspace({ nav, account, catalog }) {
  ...
  const archiveSong = async (song) => { if (!window.confirm(`Archive “${song.title || "this song"}”?`)) return; setBusy(true); setError(""); try { await archiveArtistSong(song.id); setWorkspace((current) => ({ ...current, songs: current.songs.filter((item) => item.id !== song.id) })); setMessage("Song archived."); } catch (archiveError) { setError(archiveError.message || "Unable to archive song."); } finally { setBusy(false); } };
...
function VenueManagerWorkspace({ nav, account }) {
  ...
  const archiveVenue = async (venue) => { if (!window.confirm(`Archive “${venue.name || "this venue"}”?`)) return; setBusy(true); setError(""); try { await archiveOwnedVenue(venue.id); if (editingVenue?.id === venue.id) setEditingVenue(null); await load(); } catch (err) { setError(err.message || "Unable to archive venue."); } finally { setBusy(false); } };
