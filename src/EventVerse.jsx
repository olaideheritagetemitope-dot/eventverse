...
function ArtistWorkspace({ nav, account, catalog }) {
  ...
  const refresh = async () => {
    ...
    try {
      ...
      setWorkspace(next);
      if (next.artist) {
        setProfileForm({ name: next.artist.name || "", bio: next.artist.bio || "", image_url: next.artist.image_url || "" });
