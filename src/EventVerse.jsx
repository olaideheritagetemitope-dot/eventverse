...
function ArtistWorkspace({ nav, account }) {
  ...
  const [profileForm, setProfileForm] = useState({ name: "", bio: "", image_url: "" });
  ...
  const refresh = async () => {
    ...
    try {
      ...
      setWorkspace(next);
      if (next.artist) setProfileForm({ name: next.artist.name || "", bio: next.artist.bio || "", image_url: next.artist.image_url || "" });
  ...
  const saveArtistProfile = async (event) => {
    ...
    try { const updated = await updateArtistProfile(artist.id, account.user.id, profileForm); setWorkspace((current) => ({ ...current, artist: updated })); setEditingProfile(false); setMessage("Artist profile saved."); } catch (saveError) { setError(saveError.message || "Unable to save artist profile."); } finally { setBusy(false); }
  };
  const saveSong = async (event) => {
    event.preventDefault(); if (!editingSong) return; setBusy(true); setMessage(""); setError("");
    try { const updated = await updateArtistSong(editingSong.id, artist.id, editingSong); setWorkspace((current) => ({ ...current, songs: current.songs.map((song) => song.id === updated.id ? updated : song) })); setEditingSong(null); setMessage("Song details saved."); } catch (saveError) { setError(saveError.message || "Unable to save song details."); } finally { setBusy(false); }
  };
  ...
  if (!artist && workspace && !error) return <Phone><TopBack title="Artist Workspace" onBack={nav.pop} /><div className="flex-1 flex items-center justify-center px-6 text-center" style={{ color: C.muted }}>Your account has the Artist role, but no linked artist profile is available yet.</div></Phone>;
  return <Phone>
    ...
    <div className="flex-1 overflow-y-auto px-5 pb-6">
      ...
      {workspace && tab === "Profile" && <><div className="rounded-2xl p-4 mb-4" style={{ background: C.card, border: `1px solid ${C.line}` }}><div className="flex items-center gap-3"><div className="w-16 h-16 rounded-full" style={imageStyle(artist?.image_url, `linear-gradient(135deg, ${C.wood}, ${C.green})`)} /><div><p className="text-[16px] font-semibold" style={{ color: C.ivory }}>{artist?.name}</p><p className="text-[11px]" style={{ color: C.muted }}>{artist?.verified ? "Verified artist" : "Artist profile"} · {artist?.follower_count || 0} followers</p></div></div><p className="text-[12px] mt-4 leading-5" style={{ color: C.muted }}>{artist?.bio || "Add a bio so attendees can discover your story."}</p></div><button onClick={() => setEditingProfile((value) => !value)} className="w-full py-3 rounded-2xl text-[12px] font-semibold" style={{ background: C.gold, color: C.bg }}>{editingProfile ? "Close editor" : "Edit artist profile"}</button>{editingProfile && <form onSubmit={saveArtistProfile} className="rounded-2xl p-4 mt-3" style={{ background: C.card, border: `1px solid ${C.line}` }}><Field label="Artist name" value={profileForm.name} onChange={(event) => setProfileForm((current) => ({ ...current, name: event.target.value }))} /><Field label="Bio" value={profileForm.bio} onChange={(event) => setProfileForm((current) => ({ ...current, bio: event.target.value }))} /><Field label="Image URL" value={profileForm.image_url} onChange={(event) => setProfileForm((current) => ({ ...current, image_url: event.target.value }))} /><GoldButton disabled={busy}>{busy ? "Saving..." : "Save profile"}</GoldButton></form>}</>}
      {workspace && tab === "Music" && <><div className="flex items-center justify-between mb-3"><p className="text-[14px] font-semibold" style={{ color: C.ivory }}>Your music</p><span className="text-[11px]" style={{ color: C.muted }}>{songs.length} songs</span></div>{songs.map((song) => <div key={song.id} className="rounded-2xl p-3 mb-3" style={{ background: C.card, border: `1px solid ${C.line}` }}><div className="flex items-center gap-3"><div className="w-11 h-11 rounded-xl" style={imageStyle(song.cover_url, `linear-gradient(135deg, ${C.wood}, ${C.green})`)} /><div className="flex-1"><p className="text-[13px] font-semibold" style={{ color: C.ivory }}>{song.title}</p><p className="text-[11px]" style={{ color: C.muted }}>{song.play_count || 0} plays · {song.duration_seconds ? `${Math.floor(song.duration_seconds / 60)}:${String(song.duration_seconds % 60).padStart(2, "0")}` : "Duration pending"}</p></div><button onClick={() => setEditingSong({ id: song.id, title: song.title, cover_url: song.cover_url || "" })} className="text-[11px]" style={{ color: C.gold }}>Edit</button></div>{editingSong?.id === song.id && <form onSubmit={saveSong} className="mt-3 pt-3 border-t" style={{ borderColor: C.line }}><Field label="Title" value={editingSong.title} onChange={(event) => setEditingSong((current) => ({ ...current, title: event.target.value }))} /><Field label="Cover URL" value={editingSong.cover_url} onChange={(event) => setEditingSong((current) => ({ ...current, cover_url: event.target.value }))} /><GoldButton disabled={busy}>{busy ? "Saving..." : "Save song"}</GoldButton></form>}</div>)}{!songs.length && <EmptyResourceCard label="No songs yet" description="Your music library will appear here when songs are provisioned." />}</>}
...
function AdminWorkspace({ nav, account }) {
  ...
  useEffect(() => { if (account?.roles?.includes("ADMIN")) refresh(""); }, [account?.roles]);
...
function RoleCenter({ nav, account }) {
  ...
  useEffect(() => { if (account?.roles?.some((role) => (typeof role === "string" ? role : role.code) === "SUPER_ADMIN")) loadSuperAdminAnalytics().then(setAnalytics).catch((loadError) => setError(loadError.message || "Unable to load Super Admin analytics.")); }, [account?.roles]);
...
function OrganizerEvents({ nav, account }) {
  ...
  const blank = { title: "", description: "", event_type: "", city: "", starts_at: "", ends_at: "", cover_url: "", venue_id: "", expected_attendance: "", additional_requirements: "" }; const [form, setForm] = useState(blank); const [ticket, setTicket] = useState({ name: "", price: "", capacity: "", sales_start: "", sales_end: "", maximum_per_customer: "4" }); const [artistId, setArtistId] = useState(""); const [artistQuery, setArtistQuery] = useState(""); const [artistMatches, setArtistMatches] = useState([]); const [staff, setStaff] = useState([]); const [staffQuery, setStaffQuery] = useState(""); const [staffMatches, setStaffMatches] = useState([]); const [staffResponsibility, setStaffResponsibility] = useState("GENERAL"); const [staffInstructions, setStaffInstructions] = useState("");
...
function VenueManagerOnboarding({ nav, account }) {
  const [form, setForm] = useState({ displayName: account?.profile?.full_name || "", reason: "" });
  const [application, setApplication] = useState(null);
  const [busy, setBusy] = useState(false);
  const [error, setError] = useState("");
  useEffect(() => { loadVenueManagerWorkspace(account?.id).then((data) => setApplication(data.application)).catch(() => {}); }, [account?.id]);
  const submit = async (event) => { event.preventDefault(); setBusy(true); setError(""); try { const next = await applyAsVenueManager(account.id, form.displayName, form.reason); setApplication(next); } catch (err) { setError(err.message || "Unable to submit application."); } finally { setBusy(false); } };
  return <Phone><TopBack title="Become a Venue Manager" onBack={nav.pop} /><div className="flex-1 overflow-y-auto px-5 pb-8"><p className="text-[12px] uppercase tracking-[0.16em] pt-2" style={{ color: C.gold }}>Venue operations</p><h1 className="ev-display text-[27px] mt-1" style={{ color: C.ivory }}>Manage spaces that bring events to life</h1><p className="text-[13px] leading-6 mt-3" style={{ color: C.muted }}>Apply for backend-reviewed Venue Manager access. Approval is never granted by the browser.</p>{error && <AuthMessage error={error} />}{application && <div className="rounded-2xl p-4 mt-5" style={{ background: C.card, border: `1px solid ${C.line}` }}><p className="text-[13px] font-semibold" style={{ color: C.ivory }}>Application status</p><p className="text-[24px] font-semibold mt-2" style={{ color: application.status === "APPROVED" ? C.goldSoft : C.ivory }}>{application.status}</p><p className="text-[11px] mt-2" style={{ color: C.muted }}>{application.status === "APPROVED" ? "Your Venue Manager access is active." : application.status === "REJECTED" ? (application.rejection_reason || "Application needs revision.") : "Your application is with the Atizzy team."}</p>{application.status === "APPROVED" && <div className="mt-4"><GoldButton onClick={() => nav.replace("venueManager")}>Open Venue Manager</GoldButton></div>}</div>}{(!application || application.status === "REJECTED") && <form onSubmit={submit} className="mt-5 rounded-2xl p-4" style={{ background: C.card, border: `1px solid ${C.line}` }}><Field label="Display name" value={form.displayName} onChange={(event) => setForm((current) => ({ ...current, displayName: event.target.value }))} /><label className="block text-[12px] mb-1.5" style={{ color: C.muted }}>Why do you want to manage venues?</label><textarea value={form.reason} onChange={(event) => setForm((current) => ({ ...current, reason: event.target.value }))} rows={5} className="w-full rounded-xl px-4 py-3.5 text-[14px] outline-none resize-none" style={{ background: C.card2, color: C.ivory, border: `1px solid ${C.line}` }} /><div className="mt-3"><GoldButton disabled={busy}>{busy ? "Submitting..." : "Submit application"}</GoldButton></div></form>}</div></Phone>;
}
function VenueManagerWorkspace({ nav, account }) {
  ...
  const [form, setForm] = useState(blankVenue); const [availability, setAvailability] = useState({ venue_id: "", starts_at: "", ends_at: "", status: "BLOCKED", note: "" });
  ...
  const load = async () => { try { setData(await loadVenueManagerWorkspace(account.id)); } catch (err) { setError(err.message || "Unable to load Venue Manager data."); } };
  useEffect(() => { void load(); }, [account?.id]);
  const create = async (event) => { event.preventDefault(); setBusy(true); setError(""); try { await createOwnedVenue(account.id, { ...form, capacity: Number(form.capacity), pricing: form.pricing ? { base: Number(form.pricing) } : {} }); setForm(blankVenue); await load(); } catch (err) { setError(err.message || "Unable to create venue."); } finally { setBusy(false); } };
...
export default function EventVerseApp() {
  ...
  const screens = {
    ...
    roleCenter: <RoleCenter nav={nav} account={account} />,    adminWorkspace: <AdminWorkspace nav={nav} account={account} />,
