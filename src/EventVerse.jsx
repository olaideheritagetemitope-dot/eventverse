...
import {
  ...
  Clock, Minus, Plus, Check, ShieldCheck, Home, Compass, Music2, Sparkles, Loader2,
...
const AudioController = React.forwardRef(function AudioController({ song, playing, setPlaying, userId, onProgress, onEnded }, controllerRef) {
...
function PremiumBadge({ compact = false }) {
  return <span className={`inline-flex items-center gap-1 rounded-full px-2 py-1 text-[9px] font-semibold uppercase tracking-[0.14em] ${compact ? "" : "px-2.5"}`} style={{ background: `${C.gold}22`, color: C.goldSoft, border: `1px solid ${C.gold}66` }}><Sparkles size={compact ? 10 : 11} /> Premium</span>;
}

function PremiumFeatureCard({ title, eyebrow, description, children, action }) {
  return <div className="rounded-2xl p-4 mb-3" style={{ background: C.card, border: `1px solid ${C.line}` }}><div className="flex items-start justify-between gap-3"><div><p className="text-[10px] uppercase tracking-[0.16em]" style={{ color: C.gold }}>{eyebrow}</p><p className="text-[14px] font-semibold mt-1" style={{ color: C.ivory }}>{title}</p>{description && <p className="text-[11px] mt-1" style={{ color: C.muted }}>{description}</p>}</div>{action}</div>{children}</div>;
}

function PremiumAttendeeTools({ nav, snapshot, loading, error }) {
  const radar = (snapshot?.followRadar || []).map((row, index) => toEvent({ ...row, city: row.city || "Location pending", venues: row.venue_name ? { name: row.venue_name } : null }, index));
  const planner = (snapshot?.planner || []).map((row, index) => toEvent({ ...row, city: row.city || "Location pending", venues: row.venue_name ? { name: row.venue_name } : null }, index));
  const stats = snapshot?.musicStats || {};
  const topSongs = (stats.topSongs || []).map((row) => toSong({ ...row, artists: row.artist_name ? { name: row.artist_name } : null }));
  if (loading) return <p className="py-8 text-center text-[12px]" style={{ color: C.muted }}>Loading your Premium attendee tools...</p>;
  if (error) return <AuthMessage error={error} />;
  if (!snapshot?.hasPremium) return <EmptyResourceCard label="Premium attendee tools" description="Upgrade from the Premium tab to unlock Follow Radar, your personal event planner, and music statistics." actionLabel="View Premium plans" onAction={() => nav.push("userExperience", { initialTab: "Premium" })} />;
  const features = snapshot.features || {};
  return <div className="space-y-3">
    <PremiumFeatureCard title="Follow Radar" eyebrow="Premium discovery" description="Upcoming live events connected to artists you follow.">
      {!features.follow_radar ? <p className="text-[12px] mt-4" style={{ color: C.muted }}>This feature is currently disabled by Atizzy configuration.</p> : !radar.length ? <EmptyResourceCard label="No radar events yet" description="Follow verified artists and their upcoming events will appear here." /> : <div className="mt-4 space-y-3">{radar.slice(0, 4).map((event) => <EventCard key={event.id} ev={event} onClick={() => nav.push("eventDetail", event)} />)}</div>}
    </PremiumFeatureCard>
    <PremiumFeatureCard title="Personal Event Planner" eyebrow="Premium planning" description="One live view of saved events and tickets you already hold.">
      {!features.planner ? <p className="text-[12px] mt-4" style={{ color: C.muted }}>This feature is currently disabled by Atizzy configuration.</p> : !planner.length ? <EmptyResourceCard label="Your planner is empty" description="Save an event or purchase a ticket and it will be collected here." /> : <div className="mt-3">{planner.slice(0, 8).map((event) => <button key={event.id} type="button" onClick={() => nav.push("eventDetail", event)} className="w-full text-left py-3 flex items-center justify-between gap-3" style={{ borderBottom: `1px solid ${C.line}` }}><div><p className="text-[13px] font-semibold" style={{ color: C.ivory }}>{event.title}</p><p className="text-[11px] mt-1" style={{ color: C.muted }}>{event.date} · {event.time} · {event.venue}</p></div><span className="text-[10px] uppercase" style={{ color: event.ticketed ? C.green : C.goldSoft }}>{event.ticketed ? "Ticketed" : "Saved"}</span></button>)}</div>}
    </PremiumFeatureCard>
    <PremiumFeatureCard title="Personal Music Statistics" eyebrow="Premium listening" description="Your private listening history, calculated from live playback activity.">
      {!features.personal_statistics ? <p className="text-[12px] mt-4" style={{ color: C.muted }}>This feature is currently disabled by Atizzy configuration.</p> : <><div className="grid grid-cols-2 gap-2 mt-4">{[["Plays", stats.totalPlays], ["Minutes listened", Math.round(Number(stats.totalSeconds || 0) / 60)], ["Unique songs", stats.uniqueSongs], ["Liked songs", stats.likedSongs]].map(([label, value]) => <div key={label} className="rounded-xl p-3" style={{ background: C.card2 }}><p className="text-[18px] font-semibold" style={{ color: C.goldSoft }}>{Number(value || 0).toLocaleString()}</p><p className="text-[10px] mt-1" style={{ color: C.muted }}>{label}</p></div>)}</div>{topSongs.length > 0 && <div className="mt-4"><p className="text-[11px] uppercase tracking-[0.14em]" style={{ color: C.muted }}>Top songs</p>{topSongs.slice(0, 5).map((song) => <button key={song.id} type="button" onClick={() => nav.push("musicDetail", song)} className="w-full text-left py-3 flex items-center justify-between gap-3" style={{ borderBottom: `1px solid ${C.line}` }}><div><p className="text-[12px] font-semibold" style={{ color: C.ivory }}>{song.title}</p><p className="text-[11px] mt-1" style={{ color: C.muted }}>{song.artist || "Artist pending"}</p></div><span className="text-[10px]" style={{ color: C.goldSoft }}>{Number(song.plays || 0).toLocaleString()} plays</span></button>)}</div>}</>}
    </PremiumFeatureCard>
  </div>;
}

function PremiumDiscoveryBar({ premium, onUpgrade }) {
  if (premium?.hasPremium) return <div className="mx-5 mb-5 rounded-2xl p-3 flex items-center justify-between gap-3" style={{ background: `${C.gold}12`, border: `1px solid ${C.gold}44` }}><div className="flex items-center gap-2"><PremiumBadge compact /><span className="text-[11px]" style={{ color: C.muted }}>Advanced discovery is active.</span></div><button type="button" onClick={onUpgrade} className="text-[11px]" style={{ color: C.gold }}>Use filters</button></div>;
  return <div className="mx-5 mb-5 rounded-2xl p-3 flex items-center justify-between gap-3" style={{ background: C.card, border: `1px solid ${C.line}` }}><div><p className="text-[12px] font-semibold" style={{ color: C.ivory }}>Unlock advanced discovery</p><p className="text-[11px] mt-1" style={{ color: C.muted }}>Filter live events by category, price, and distance with Premium.</p></div><button type="button" onClick={onUpgrade} className="text-[11px] font-semibold" style={{ color: C.gold }}>View Premium</button></div>;
}

function PremiumDiscoveryFilters({ premium, categories, onResults }) {
  ...
  const [busy, setBusy] = useState(false);
  const [message, setMessage] = useState("");
  if (!premium?.hasPremium) return null;
  const apply = async () => {
    setBusy(true); setMessage("");
    try {
      let latitude = null; let longitude = null;
      if (radiusKm && typeof navigator !== "undefined" && navigator.geolocation) {
        const position = await new Promise((resolve) => navigator.geolocation.getCurrentPosition(resolve, () => resolve(null), { enableHighAccuracy: false, maximumAge: 300000, timeout: 5000 }));
        if (position?.coords) { latitude = position.coords.latitude; longitude = position.coords.longitude; } else setMessage("Location permission was not available, so distance was not applied. You can still use category and price filters.");
      }
      const rows = await loadPremiumEventDiscovery({ category: category === "All" ? "" : category, minPrice, maxPrice, radiusKm, latitude, longitude });
      onResults(rows.map((row, index) => toEvent({ ...row, venues: row.venue_name ? { name: row.venue_name } : null }, index)));
    } catch (error) { setMessage(error.message || "Premium discovery filters are unavailable."); } finally { setBusy(false); }
  };
  return <div className="mx-5 mb-5 rounded-2xl p-4" style={{ background: C.card, border: `1px solid ${C.gold}44` }}><div className="flex items-center justify-between"><div><div className="flex items-center gap-2"><PremiumBadge compact /><p className="text-[13px] font-semibold" style={{ color: C.ivory }}>Advanced event filters</p></div><p className="text-[11px] mt-1" style={{ color: C.muted }}>Server-authorized filters for live upcoming events.</p></div><span className="text-[10px] uppercase" style={{ color: C.goldSoft }}>Premium</span></div><div className="grid grid-cols-2 gap-2 mt-4"><select value={category} onChange={(event) => setCategory(event.target.value)} className="rounded-xl px-3 py-2 text-[11px]" style={{ background: C.card2, color: C.ivory, border: `1px solid ${C.line}` }}><option value="All">All categories</option>{(categories || []).filter((item) => item.name && item.name !== "All").map((item) => <option key={item.name} value={item.name}>{categoryLabel(item)}</option>)}</select><input value={radiusKm} onChange={(event) => setRadiusKm(event.target.value.replace(/[^0-9.]/g, ""))} placeholder="Distance km" inputMode="decimal" className="rounded-xl px-3 py-2 text-[11px] outline-none" style={{ background: C.card2, color: C.ivory, border: `1px solid ${C.line}` }} /><input value={minPrice} onChange={(event) => setMinPrice(event.target.value.replace(/[^0-9.]/g, ""))} placeholder="Min price" inputMode="decimal" className="rounded-xl px-3 py-2 text-[11px] outline-none" style={{ background: C.card2, color: C.ivory, border: `1px solid ${C.line}` }} /><input value={maxPrice} onChange={(event) => setMaxPrice(event.target.value.replace(/[^0-9.]/g, ""))} placeholder="Max price" inputMode="decimal" className="rounded-xl px-3 py-2 text-[11px] outline-none" style={{ background: C.card2, color: C.ivory, border: `1px solid ${C.line}` }} /></div><button type="button" onClick={apply} disabled={busy} className="w-full mt-3 py-2.5 rounded-xl text-[12px] font-semibold disabled:opacity-50" style={{ background: C.gold, color: C.bg }}>{busy ? "Filtering live events..." : "Apply Premium filters"}</button>{message && <p className="text-[11px] mt-2" style={{ color: message.startsWith("Location") ? C.goldSoft : C.red }}>{message}</p>}</div>;
}
...
function AttendeeHome({ nav, player, catalog, account, loading, error, catalogNotice, premium, locationState = "unavailable", onRetryLocation }) {
  ...
  const liveLocation = locationState === "granted" ? "Nearby you" : locationState === "denied" ? "Location off" : locationState === "timeout" ? "Location timed out" : "Location unavailable";
  ...
  return (
    <Phone>
      ...
      <div className="flex-1 overflow-y-auto pb-2">
        ...
        <PremiumDiscoveryBar premium={premium} onUpgrade={() => nav.push("userExperience", { initialTab: "Premium" })} />
        {catalogNotice && <p className="text-[10px] mx-5 mb-3 px-3 py-2 rounded-xl" style={{ color: C.goldSoft, background: `${C.gold}12`, border: `1px solid ${C.gold}33` }} role="status">Some live sections could not refresh. Showing the available live data.</p>}
...
function Explore({ nav, player, catalog, catalogNotice, premium }) {
  ...
  return (
    <Phone>
      ...
      {catalogNotice && <p className="text-[10px] mx-5 mb-3 px-3 py-2 rounded-xl" style={{ color: C.goldSoft, background: `${C.gold}12`, border: `1px solid ${C.gold}33` }} role="status">Some live sections could not refresh. Showing the available live data.</p>}
      <PremiumDiscoveryFilters premium={premium} categories={categories} onResults={setFilteredEvents} />

      <div className="flex-1 overflow-y-auto">
        {filteredEvents !== null && <Section title="Premium Filter Results" nav={nav}>
          {filteredEvents.map((ev) => <EventCard key={ev.id} ev={ev} onClick={() => nav.push("eventDetail", ev)} />)}
          {!filteredEvents.length && <EmptyEventCard />}
        </Section>}
...
function SearchScreen({ nav, catalog, account }) {
  ...
  const artists = query && show("Artists") ? results.artists : [];
  const songs = query && show("Songs") ? results.songs : [];
  const events = query && show("Events") ? results.events : [];
  const venues = query && show("Venues") ? results.venues : [];
  return (
    <Phone>
      ...
      <div className="flex-1 overflow-y-auto px-5">
        ...
        {!loading && !error && query && !artists.length && !songs.length && !events.length && !venues.length && <p className="text-[13px] py-8 text-center" style={{ color: C.muted }}>No results for “{query}” in {tab === "All" ? "the live catalog" : tab.toLowerCase()}.</p>}
...
function ArtistWorkspace({ nav, account, catalog }) {
  ...
  const saveSong = async (event, publishAfterSave = false) => {
    ...
    try {
      ...
      const published = publishAfterSave && saved.status !== "PUBLISHED" ? await setArtistSongStatus(saved.id, "PUBLISHED") : saved;
      const nextSong = { ...saved, ...(Array.isArray(published) ? published[0] : published), status: publishAfterSave ? "PUBLISHED" : saved.status };
      setWorkspace((current) => ({ ...current, songs: editingSong.id ? current.songs.map((song) => song.id === nextSong.id ? nextSong : song) : [nextSong, ...current.songs] }));
      setSongCoverFile(null); setSongAudioFile(null); setSongMusicVideoFile(null); setEditingSong(null); setMessage(publishAfterSave ? "Song posted to the live catalog." : (editingSong.id ? "Song details saved." : "Song saved as a draft."));
...
function GovernanceDashboard({ nav, account, onAccountUpdated }) {
  ...
  const [directory, setDirectory] = useState({ rows: [], totalCount: 0 });
  const [roleTarget, setRoleTarget] = useState(null);
  const [roleCode, setRoleCode] = useState("ARTIST");
  const [roleAction, setRoleAction] = useState("ASSIGN");
  const [roleReason, setRoleReason] = useState("");
  const refresh = async (directoryOffset = 0) => { setBusy(true); try { const [data, eventRows, historyRows, directoryPage] = await Promise.all([loadRoleGovernanceSnapshot(), loadGovernanceEvents(), loadRoleAssignmentHistory(), adminListUsersPage({ limit: 50, offset: directoryOffset })]); setSnapshot(data); setEvents(eventRows); setRoleHistory(historyRows); setDirectory(directoryPage); setFee(data.fees?.[0] || null); } catch (error) { setNotice(error.message || "Unable to load governance data."); } finally { setBusy(false); } };
  useEffect(() => { refresh(); }, []);
...
function AdminControlCenter({ nav, account }) {
  ...
  const liveModules = [
    ...
    ["Legal / Policies", "Create, preview, publish, archive, restore, and audit legal versions", "adminLegal"],
...
function OrganizerEvents({ nav, account }) {
  ...
  const choose = async (event) => { setSelected(event); setLocationMode(event.venue_id ? "venue" : "custom"); setVenueQuery(""); setForm({ title: event.title || "", description: event.description || "", event_type: event.event_type || "", city: event.city || "", starts_at: event.starts_at ? event.starts_at.slice(0, 16) : "", ends_at: event.ends_at ? event.ends_at.slice(0, 16) : "", cover_url: event.cover_url || "", venue_id: event.venue_id || "", latitude: event.latitude ?? "", longitude: event.longitude ?? "", formatted_address: event.formatted_address || event.venues?.formatted_address || event.venues?.address || "", state_region: event.state_region || event.venues?.state_region || "", country: event.country || event.venues?.country || "", provider_place_id: event.provider_place_id || event.venues?.provider_place_id || "", location_metadata: event.location_metadata || {}, location_confirmed: Boolean(event.venue_id) || Boolean(event.location_metadata?.location_confirmed), expected_attendance: "", additional_requirements: "" }); setMode("detail"); try { const [nextDashboard, nextStaff] = await Promise.all([loadOrganizerEventDashboard(event.id), loadEventStaffForOrganizer(event.id)]); setDashboard(nextDashboard); setStaff(nextStaff); } catch (loadError) { setError(loadError.message || "Unable to load event analytics."); } };
  const selectVenue = (venue) => { if (!venue) return; setLocationMode("venue"); setVenueQuery(venue.name || ""); setForm((current) => ({ ...current, venue_id: venue.id, latitude: venue.latitude ?? "", longitude: venue.longitude ?? "", formatted_address: venue.formatted_address || venue.address || "", state_region: venue.state_region || "", country: venue.country || "", provider_place_id: venue.provider_place_id || "", location_metadata: venue.location_metadata || {}, location_confirmed: true })); };
  const useCustomLocation = () => { setLocationMode("custom"); setVenueQuery(""); setForm((current) => ({ ...current, venue_id: "", latitude: "", longitude: "", formatted_address: "", state_region: "", country: "", provider_place_id: "", location_metadata: { source: "custom_event_location" }, location_confirmed: false })); };
  const saveEvent = async (event) => { event.preventDefault(); setBusy(true); setError(""); try { if (locationMode === "venue" && !form.venue_id) throw new Error("Select an existing venue or choose a custom location."); if (locationMode === "custom" && (!form.location_confirmed || !form.latitude || !form.longitude)) throw new Error("Search and confirm the custom event location before saving."); const cover = coverFile ? await uploadMediaFile(account.user.id, coverFile, "EVENT_POSTER", "events", selected?.id || null) : null; const payload = { ...form, cover_url: cover?.public_url || form.cover_url, location_metadata: locationMode === "custom" ? { ...(form.location_metadata || {}), source: "custom_event_location", location_confirmed: true } : (form.location_metadata || {}) }; const next = selected ? await updateOrganizerEvent(selected.id, account.user.id, payload) : await createOrganizerEvent(account.user.id, payload); setMessage(selected ? "Event draft saved with its location." : "Draft event created with its location attached."); await refresh(); setSelected(next); setMode("detail"); } catch (saveError) { setError(saveError.message || "Unable to save event location."); } finally { setBusy(false); } };
  const addTicket = async (event) => { event.preventDefault(); if (!selected) return; setBusy(true); setError(""); try { const createdTicket = await addOrganizerTicketType(selected.id, account.user.id, ticket); if (createdTicket?.id) await updateOrganizerTicketReleasePolicy(createdTicket.id, ticket); setTicket({ name: "", price: "", capacity: "", sales_start: "", sales_end: "", maximum_per_customer: "4", visibility: "PUBLIC", access_method: "CODE", code: "", word: "", access_credential_hint: "", maximum_redemptions: "", maximum_purchases_per_user: "", premium_early_access_starts_at: "", public_release_at: "", premium_only: false }); await refresh(); setMessage("Ticket type added with server-side inventory and release policy."); } catch (ticketError) { setError(ticketError.message || "Unable to add ticket type."); } finally { setBusy(false); } };
...
function TomTomVenueLocationPicker({ value, onChange }) {
  ...
  return <div className="mb-4 rounded-2xl p-3" style={{ background: C.card2, border: `1px solid ${C.line}` }}>
    <p className="text-[12px] font-semibold mb-2" style={{ color: C.ivory }}>Search and confirm exact location</p>
        <div role="search" className="flex gap-2"><input value={query} onChange={(event) => setQuery(event.target.value)} onKeyDown={(event) => { if (event.key === "Enter") { event.preventDefault(); void search(); } }} placeholder="Search venue or address" className="min-w-0 flex-1 rounded-xl px-3 py-3 text-[13px] outline-none" style={{ background: C.card, color: C.ivory, border: `1px solid ${C.line}` }} /><button type="button" onClick={() => void search()} disabled={busy} className="rounded-xl px-3 text-[12px] font-semibold" style={{ background: C.gold,
 color: C.bg }}>{busy ? "..." : "Search"}</button></div>
...
function VenueManagerWorkspace({ nav, account }) {
  ...
  const isSuperAdmin = hasEffectiveRole(account, "SUPER_ADMIN");
  const blankVenue = { name: "", city: "", address: "", capacity: "", description: "", venue_type: "", rules: "", contact_phone: "", cancellation_policy: "", pricing: "", latitude: "", longitude: "", formatted_address: "", state_region: "", country: "", provider_place_id: "" };
...
/* ============================== USER EXPERIENCE ============================== */
function PremiumPanel({ account }) {
  const [snapshot, setSnapshot] = useState({ plans: [], subscription: null, entitlement: null, hasPremium: false });
  const [loading, setLoading] = useState(true);
  const [busy, setBusy] = useState(false);
  const [message, setMessage] = useState("");
  const refresh = async () => { setLoading(true); setMessage(""); try { setSnapshot(await loadPremiumSnapshot()); } catch (error) { setMessage(error.message || "Unable to load Premium status."); } finally { setLoading(false); } };
  useEffect(() => { refresh(); }, []);
  const checkout = async (plan) => { setBusy(true); setMessage(""); try { const key = typeof crypto !== "undefined" && crypto.randomUUID ? crypto.randomUUID() : `${Date.now()}-${Math.random().toString(36).slice(2)}`; const callbackUrl = `${window.location.origin}/?premium-payment=callback`; const result = await initializePremiumPayment(plan.id, key, account?.user?.email, callbackUrl); if (result.authorizationUrl) { window.localStorage.setItem("atizzy:pending-premium-payment", JSON.stringify({ paymentId: result.paymentId, reference: result.reference || result.transactionReference, transactionReference: result.transactionReference, amount: result.amount, currency: result.currency })); window.location.assign(result.authorizationUrl); } else setMessage("Premium checkout was initialized, but no checkout URL was returned."); } catch (error) { setMessage(error.message || "Unable to start Premium checkout."); } finally { setBusy(false); } };
...
function PremiumProcessing({ nav, data }) {
  const [status, setStatus] = useState("PROCESSING");
  const [message, setMessage] = useState("");
  useEffect(() => { let mounted = true; let timer; const verify = async () => { try { const session = (await supabase.auth.getSession()).data.session; if (!session?.access_token) throw new Error("Please sign in again to verify Premium payment."); const response = await fetch(apiUrl("/api/paystack/premium-verify"), { method: "POST", headers: { "Content-Type": "application/json", Authorization: `Bearer ${session.access_token}` }, body: JSON.stringify({ paymentId: data?.paymentId, reference: data?.reference }) }); const payload = await response.json().catch(() => ({})); if (!response.ok) throw new Error(payload.error || "Premium payment verification failed."); if (!mounted) return; setStatus(payload.verified ? "VERIFIED_SUCCESS" : "PROCESSING"); if (payload.verified) { window.localStorage.removeItem("atizzy:pending-premium-payment"); timer = window.setTimeout(() => nav.replace("userExperience", { initialTab: "Premium" }), 700); } else timer = window.setTimeout(verify, 2500); } catch (error) { if (!mounted) return; setMessage(error.message || "Unable to verify Premium payment."); timer = window.setTimeout(verify, 5000); } }; verify(); return () => { mounted = false; window.clearTimeout(timer); }; }, [data?.paymentId, data?.reference]);
  return <Phone><div className="flex-1 flex flex-col justify-center px-6 text-center"><div className="mx-auto w-16 h-16 rounded-full flex items-center justify-center" style={{ background: C.green, border: `1px solid ${C.gold}` }}><Clock size={26} color={C.gold} /></div><h1 className="ev-display text-[27px] mt-5" style={{ color: C.ivory }}>{status === "VERIFIED_SUCCESS" ? "Premium is active" : "Confirming Premium"}</h1><p className="text-[13px] leading-6 mt-3" style={{ color: C.muted }}>{status === "VERIFIED_SUCCESS" ? "Your Premium entitlement is active. Restoring your account..." : "Paystack returned. Atizzy is verifying the signed payment before granting access."}</p>{message && <p className="text-[12px] mt-4" style={{ color: C.red }}>{message}</p>}<div className="mt-6"><GhostButton onClick={() => nav.replace("userExperience", { initialTab: "Premium" })}>Back to Premium</GhostButton></div></div></Phone>;
}
function UserExperience({ nav, account, initialTab = "Preferences", premium, premiumLoading, premiumError }) {
  ...
  return <Phone>
    ...
    <div className="flex gap-2 px-5 pb-4 overflow-x-auto no-scrollbar">{["Preferences", "Notifications", "Premium", "Search history", "Help & Support"].map((item) => <Pill key={item} active={tab === item} onClick={() => setTab(item)}>{item}</Pill>)}</div>
    <div className="flex-1 overflow-y-auto px-5 pb-4">{loading && <p className="text-[12px] py-8 text-center" style={{ color: C.muted }}>Loading your Atizzy settings...</p>}
      ...
      {!loading && tab === "Premium" && <><PremiumPanel account={account} /><PremiumAttendeeTools nav={nav} snapshot={premium} loading={premiumLoading} error={premiumError} /></>}
...
function Profile({ nav, player, account, premium, onAccountUpdated }) {
  const items = ["My Tickets", "Music Library", "Followed Artists", "Liked Music", "Recently Played", "Activity", "Preferences", "Notifications", "Premium", "Security", "Help & Support"];
  const [busy, setBusy] = useState(false);
  ...
  return (
    <Phone>
      <div className="px-5 pt-2 pb-4 flex flex-col items-center">
        ...
        <div className="flex items-center gap-2"><p className="text-[16px] font-semibold" style={{ color: C.ivory }}>{account?.profile?.full_name || account?.user?.email || "Atizzy member"}</p>{premium?.hasPremium && <PremiumBadge compact />}</div>
      ...
      <div className="flex-1 overflow-y-auto px-5">
        ...
        {hasEffectiveRole(account, "ARTIST") && <button aria-label="Golden verification active when verified" onClick={() => nav.push("artistVerification")} className="w-full flex items-center justify-between py-3.5" style={{ borderBottom: `1px solid ${C.line}` }}><span className="text-[13.5px]" style={{ color: C.goldSoft }}>Get Verified</span><ShieldCheck size={15} color={C.gold} /></button>}<button onClick={() => nav.push("legalPrivacy")} className="w-full flex items-center justify-between py-3.5" style={{ borderBottom: `1px solid ${C.line}` }}><span className="text-[13.5px]" style={{ color: C.goldSoft }}>Privacy Policy</span><ChevronRight size={15} color={C.muted} /></button><button onClick={() => nav.push("legalTerms")} className="w-full flex items-center justify-between py-3.5" style={{ borderBottom: `1px solid ${C.line}` }}><span className="text-[13.5px]" style={{ color: C.goldSoft }}>Terms & Conditions</span><ChevronRight size={15} color={C.muted} /></button>
        {items.map((it) => (
          <button key={it} onClick={() => it === "My Tickets" ? nav.push("tickets") : it === "Music Library" ? nav.push("music") : ["Followed Artists", "Liked Music", "Recently Played", "Activity"].includes(it) ? nav.push("profileCollections", { initialTab: it }) : ["Preferences", "Notifications", "Premium", "Help & Support"].includes(it) ? nav.push("userExperience", { initialTab: it }) : it === "Security" ? nav.push("security") : null} className="w-full flex items-center justify-between py-3.5" style={{ borderBottom: `1px solid ${C.line}` }}>
...
function PlaylistDetail({ nav, account, player, catalog, data }) {
  ...
  const canEdit = owner || Boolean(playlist?.visibility === "PUBLIC" && playlist?.public_edit_enabled === true);
  const refresh = async () => { setLoading(true); try { const result = await loadPlaylist(data?.id); setPlaylist(result); setName(result.name); setDescription(result.description || ""); setVisibility(result.visibility || "PRIVATE"); setPublicEditEnabled(result.public_edit_enabled === true); } catch (error) { setMessage(error.message || "Unable to load this playlist."); } finally { setLoading(false); } };
  useEffect(() => { void refresh(); }, [data?.id]);
  const save = async (nextVisibility = visibility) => { if (!account?.user?.id || !playlist?.id) return; setSaving(true); try { const updated = nextVisibility === "PUBLIC" && owner ? await publishPlaylist(account.user.id, playlist.id, { name, description, public_edit_enabled: publicEditEnabled }) : await updatePlaylist(account.user.id, playlist.id, { name, description, visibility: owner ? nextVisibility : undefined, public_edit_enabled: owner ? publicEditEnabled : undefined }); setPlaylist(updated); setName(updated.name || name); setDescription(updated.description || ""); setVisibility(String(updated.visibility || nextVisibility).toUpperCase()); setPublicEditEnabled(updated.public_edit_enabled === true); setMessage(String(updated.visibility || "").toUpperCase() === "PUBLIC" ? "Playlist published publicly." : "Playlist saved privately."); } catch (error) { setMessage(error.message || "Unable to update playlist."); } finally { setSaving(false); } };
  const uploadCover = async (event) => { const file = event.target.files?.[0]; event.target.value = ""; if (!file || !account?.user?.id || !playlist?.id) return; setUploadingCover(true); try { const asset = await uploadMediaFile(account.user.id, file, "PLAYLIST_COVER", "PLAYLIST", playlist.id); const updated = await updatePlaylist(account.user.id, playlist.id, { cover_url: asset.public_url }); setPlaylist(updated); setMessage("Playlist cover uploaded."); } catch (error) { setMessage(error.message || "Unable to upload playlist cover."); } finally { setUploadingCover(false); } };
  ...
  return <Phone><div className="flex-1 overflow-y-auto"><div className="px-5 pt-4 pb-4"><button type="button" onClick={nav.pop} className="mb-4"><ChevronLeft size={20} color={C.ivory} /></button><div className="w-full h-36 rounded-2xl mb-4" style={imageStyle(playlist?.cover_url, `linear-gradient(135deg, ${C.wood}, ${C.green})`)} /><h1 className="ev-display text-[24px]" style={{ color: C.ivory }}>{playlist?.name}</h1><p className="text-[11px] mt-1" style={{ color: C.muted }}>{playlist?.visibility === "PUBLIC" ? "Public playlist" : "Private playlist"} · {playlist?.items?.length || 0} songs{playlist?.public_edit_enabled ? " · Public edits open" : ""}</p>{playlist?.description && <p className="text-[12px] mt-2" style={{ color: C.muted }}>{playlist.description}</p>}<div className="flex gap-2 mt-4"><button type="button" onClick={() => playAll(false)} disabled={!playlist?.items?.length} className="rounded-xl px-4 py-2 text-[11px] disabled:opacity-40" style={{ background: C.gold, color: "#1A1408" }}>Play All</button><button type="button" onClick={() => playAll(true)} disabled={!playlist?.items?.length} className="rounded-xl px-4 py-2 text-[11px] disabled:opacity-40" style={{ background: C.card, color: C.ivory, border: `1px solid ${C.line}` }}>Shuffle</button></div></div>{canEdit && <div className="mx-5 mb-5 rounded-2xl p-3" style={{ background: C.card, border: `1px solid ${C.line}` }}><p className="text-[12px] font-semibold mb-2" style={{ color: C.ivory }}>Playlist settings{!owner && " · Public editor"}</p><input value={name} onChange={(event) => setName(event.target.value)} className="w-full rounded-lg px-2 py-2 text-[11px] mb-2" style={{ background: C.panel, color: C.ivory, border: `1px solid ${C.line}` }} /><textarea value={description} onChange={(event) => setDescription(event.target.value)} placeholder="Description (optional)" className="w-full rounded-lg px-2 py-2 text-[11px] mb-2" style={{ background: C.panel, color: C.ivory, border: `1px solid ${C.line}` }} /><div className="flex flex-wrap gap-2 items-center">{owner && <><select value={visibility} onChange={(event) => setVisibility(event.target.value)} className="rounded-lg px-2 py-2 text-[10px]" style={{ background: C.panel, color: C.ivory, border: `1px solid ${C.line}` }}><option value="PRIVATE">Private</option><option value="PUBLIC">Public</option></select><label className="flex items-center gap-1 rounded-lg px-2 py-2 text-[10px]" style={{ background: C.panel, color: C.ivory, border: `1px solid ${C.line}` }}><input type="checkbox" checked={publicEditEnabled} onChange={(event) => setPublicEditEnabled(event.target.checked)} disabled={visibility !== "PUBLIC"} /> Allow public edits</label></>}<button type="button" onClick={() => save(visibility)} disabled={saving} className="rounded-lg px-3 py-2 text-[10px] disabled:opacity-50" style={{ background: C.gold, color: "#1A1408" }}>{saving ? "Saving…" : visibility === "PUBLIC" ? "Publish playlist" : "Save privately"}</button>{owner && <><label className="rounded-lg px-3 py-2 text-[10px] cursor-pointer" style={{ background: C.panel, color: C.ivory, border: `1px solid ${C.line}` }}>{uploadingCover ? "Uploading…" : "Upload cover photo"}<input type="file" accept="image/jpeg,image/png,image/webp,image/gif" onChange={uploadCover} disabled={uploadingCover} className="hidden" /></label><button type="button" onClick={destroy} className="rounded-lg px-3 py-2 text-[10px]" style={{ background: C.red, color: C.ivory }}>Delete</button></>}</div></div>}<div className="px-5 mb-5"><p className="text-[14px] font-semibold mb-2" style={{ color: C.ivory }}>Songs</p>{playlist?.items?.map((item, index) => <div key={item.id || item.song_id} className="flex items-center gap-2 py-2" style={{ borderBottom: `1px solid ${C.line}` }}><button type="button" onClick={() => { const song = playlistSongForPlayer(item); if (song) player.play(song, playlist.items.map(playlistSongForPlayer).filter(Boolean)); }} className="flex-1 min-w-0 text-left"><p className="text-[12px] truncate" style={{ color: C.ivory }}>{item.song?.title || "Unavailable song"}</p><p className="text-[10px] truncate" style={{ color: C.muted }}>{item.song?.artists?.name || item.song?.artist || "Artist pending"}</p></button>{canEdit && <><button type="button" aria-label="Move song up" onClick={() => reorder(index, -1)} className="p-1"><ChevronDown size={14} color={index ? C.muted : C.line} style={{ transform: "rotate(180deg)" }} /></button><button type="button" aria-label="Move song down" onClick={() => reorder(index, 1)} className="p-1"><ChevronDown size={14} color={index < playlist.items.length - 1 ? C.muted : C.line} /></button><button type="button" aria-label="Remove song" onClick={() => remove(item.song_id)} className="p-1"><X size={14} color={C.red} /></button></>}</div>)}{!playlist?.items?.length && <p className="text-[11px] py-3" style={{ color: C.muted }}>This playlist is empty. Add songs from the Music library.</p>}</div>{canEdit && <div className="px-5 pb-6"><p className="text-[12px] font-semibold mb-2" style={{ color: C.ivory }}>Add Songs</p>{availableSongs.slice(0, 12).map((song) => <div key={song.id} className="flex items-center gap-2 py-2" style={{ borderBottom: `1px solid ${C.line}` }}><div className="w-8 h-8 rounded-lg" style={imageStyle(song.coverUrl, `linear-gradient(135deg, ${C.wood}, ${C.green})`)} /><div className="min-w-0 flex-1"><p className="text-[11px] truncate" style={{ color: C.ivory }}>{song.title}</p><p className="text-[10px] truncate" style={{ color: C.muted }}>{song.artist}</p></div><button type="button" onClick={() => add(song)} className="rounded-lg px-2 py-1 text-[10px]" style={{ background: C.gold, color: "#1A1408" }}>Add</button></div>)}</div>}{message && <p className="px-5 pb-5 text-[11px]" style={{ color: C.muted }}>{message}</p>}</div><MiniPlayer song={player.song} playing={player.playing} onToggle={player.toggle} onPrevious={player.previous} onNext={player.next} onOpen={() => nav.push("musicPlayer")} /></Phone>;
}
...
function FullPlayer({ nav, player, account }) {
  ...
  const toggleLike = async () => {
    ...
    const next = !liked;
    try { await toggleMusicFavorite(account.user.id, song.id, next); setLiked(next); setPlayerMessage(""); } catch (toggleError) { setPlayerMessage(toggleError.message || "Unable to update favorite."); }
  };
  const share = async () => {
        ...
        <div className="flex-1 min-h-0 overflow-hidden">
          <div className="flex h-full transition-transform duration-300 ease-out" style={{ width: "300%", transform: `translateX(-${page * (100 / 3)}%)` }}>
            <div className="h-full min-w-0 w-1/3 flex-shrink-0 flex flex-col px-6 overflow-y-auto">
              ...
              {playerMessage && <p className="text-[11px] mb-3" role="status" style={{ color: C.red }}>{playerMessage}</p>}
...
function MusicVideoDetail({ nav, video, player, account }) {
  const initialVideo = video ? toMusicVideo(video) : null;
  ...
  useEffect(() => {
    ...
    if (video?.id) loadMusicVideoDetail(video.id).then((liveVideo) => { if (active && liveVideo) setResolvedVideo(liveVideo); }).catch(() => {});
    return () => { active = false; };
  }, [video?.id]);
  ...
  const toggleLike = async () => {
    ...
    const next = !liked;
    setLiked(next);
    try { await toggleMusicVideoFavorite(account.user.id, currentVideo.id, next); setMessage(""); }
...
function MusicDetail({ nav, data, player, account }) {
  ...
  useEffect(() => { if (!data?.videoUrl && !data?.musicVideoUrl && account?.user?.id && song?.id) loadFavoriteState(account.user.id, null, null, song.id, null).then((state) => setLiked(Boolean(state.musicFavorite))).catch(() => {}); }, [account?.user?.id, song?.id, data?.videoUrl, data?.musicVideoUrl]);
...
function SecurityScreen({ nav, account }) {
  ...
  const deleteAccount = async () => {
    if (!window.confirm("Delete your Eventverse account permanently? This cannot be undone.")) return;
    setBusy(true); setMessage("");
    try { await deleteMyAccount(); nav.reset("login"); }
    catch (error) { setMessage(error.message || "The account could not be deleted."); setBusy(false); }
  };
  return <Phone><TopBack title="Security" onBack={nav.pop} /><div className="flex-1 overflow-y-auto px-5 pb-6"><div className="rounded-2xl p-4 mb-4" style={{ background: C.card }}><div className="flex items-center gap-3"><ShieldCheck size={22} color={C.gold} /><div><p className="font-semibold text-[14px]" style={{ color: C.ivory }}>Account security</p><p className="text-[11px] mt-1" style={{ color: C.muted }}>Your session is protected by Eventverse authentication.</p></div></div></div><p className="text-[12px] font-semibold mb-2" style={{ color: C.muted }}>AUTHENTICATION METHODS</p><div className="rounded-2xl p-4 mb-4" style={{ background: C.card }}><p className="text-[13px]" style={{ color: C.ivory }}>{account?.user?.email || "Authenticated account"}</p><p className="text-[11px] mt-1" style={{ color: C.muted }}>Primary email login</p></div><p className="text-[12px] font-semibold mb-2" style={{ color: C.muted }}>ACTIVE SESSION</p><div className="rounded-2xl p-4 mb-4" style={{ background: C.card }}><p className="text-[13px]" style={{ color: C.ivory }}>Current device</p><p className="text-[11px] mt-1" style={{ color: C.muted }}>This session is active and monitored.</p></div><section className="rounded-2xl p-4" style={{ background: C.card, border: `1px solid #E9897955` }}><p className="text-[12px] font-semibold" style={{ color: "#E98979" }}>DELETE ACCOUNT</p><p className="text-[11px] leading-5 mt-2" style={{ color: C.muted }}>Permanently delete your Eventverse account and associated profile data. This action cannot be undone.</p><button type="button" onClick={deleteAccount} disabled={busy} className="mt-3 rounded-xl px-3 py-2 text-[12px] font-semibold" style={{ background: "#E98979", color: C.bg }}>{busy ? "Deleting account..." : "Delete my account"}</button>{message && <p className="text-[11px] mt-2" style={{ color: "#E98979" }}>{message}</p>}</section></div></Phone>;
}
...
export default function EventVerseApp() {
  const [stack, setStack] = useState(() => {
    ...
    const legalScreen = typeof window !== "undefined" && window.location.pathname === "/privacy-policy" ? "legalPrivacy" : (typeof window !== "undefined" && window.location.pathname === "/terms-and-conditions" ? "legalTerms" : null);
    return [{ screen: legalScreen || (sharedEventId ? "eventDetail" : (sharedArtistId ? "artist" : (completed ? "login" : "onboarding"))), data: sharedEventId ? { id: decodeURIComponent(sharedEventId) } : (sharedArtistId ? { id: decodeURIComponent(sharedArtistId) } : null) }];
  });
  ...
  const accountHydrationRef = useRef(null);
  const hydrateAccount = (user) => {
    if (!user?.id) {
      const empty = { user: null, profile: null, roles: [], effectiveRoles: [], primaryRole: null };
      setAccount(empty);
      return Promise.resolve(empty);
    }
    const active = accountHydrationRef.current;
    if (active?.userId === user.id) return active.promise;
    const promise = (async () => {
      ...
      console.error("EventVerse account hydration failed", error);
      return null;
    }).finally(() => {
      if (accountHydrationRef.current?.promise === promise) accountHydrationRef.current = null;
    });
    accountHydrationRef.current = { userId: user.id, promise };
    return promise;
  };
  const refreshAccount = () => hydrateAccount(account?.user);

  useEffect(() => {
    let mounted = true;
    const loadPremium = async () => {
      if (!account?.user?.id) {
        setPremium({ hasPremium: false, features: {}, followRadar: [], planner: [], musicStats: { totalPlays: 0, totalSeconds: 0, uniqueSongs: 0, likedSongs: 0, topSongs: [] } });
        setPremiumError("");
      ...
      setPremiumError("");
      try {
        const entitlement = await loadPremiumSnapshot();
        if (!entitlement?.hasPremium) {
          if (mounted) setPremium({ ...entitlement, hasPremium: false, features: {}, followRadar: [], planner: [], musicStats: { totalPlays: 0, totalSeconds: 0, uniqueSongs: 0, likedSongs: 0, topSongs: [] } });
          return;
        }
        const attendee = await loadPremiumAttendeeSnapshot();
        if (mounted) setPremium({ ...attendee, hasPremium: true, subscription: entitlement.subscription, entitlement: entitlement.entitlement });
      } catch (error) {
        if (mounted) setPremiumError(error.message || "Unable to load Premium attendee tools.");
      } finally {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      let discovery = null;
        ...
        if (mounted) {
          const partialErrors = Object.keys(liveCatalog.catalogErrors || {});
          setCatalogNotice(partialErrors.length ? `Unavailable live sections: ${partialErrors.join(", ")}` : "");
    ...
    const restore = async () => {
      try {
        ...
        const pendingPremiumPayment = premiumPaymentCallback ? JSON.parse(window.localStorage.getItem("atizzy:pending-premium-payment") || "null") : null;
        if (paymentCallback || artistPaymentCallback || premiumPaymentCallback) {
          url.searchParams.delete("payment");
          url.searchParams.delete("artist-payment");
          url.searchParams.delete("premium-payment");
          window.history.replaceState({}, document.title, `${url.pathname}${url.search}`);
        }
    ...
    const { data: listener } = supabase.auth.onAuthStateChange((event, session) => {
      ...
      setTimeout(() => { void hydrateAccount(session.user); }, 0);
    ...
    load();
    // Render the auth shell immediately. Session restoration and account hydration continue
    // in the background; protected workspaces still depend on the verified account state.
    setAuthReady(true);
    void restore();
  ...
  const screens = {
    ...
    artistDirectory: <ArtistDirectory nav={nav} player={player} catalog={catalog} />, legalPrivacy: <LegalDocumentScreen nav={nav} documentType="PRIVACY_POLICY" />, legalTerms: <LegalDocumentScreen nav={nav} documentType="TERMS_AND_CONDITIONS" />, adminLegal: <AdminLegalCMS nav={nav} account={account} />,
    ...
    tickets_tab: null,
    profile: <Profile nav={nav} player={player} account={account} premium={premium} onAccountUpdated={(profile) => setAccount((current) => ({ ...current, profile: { ...current.profile, ...profile } }))} />,
    userExperience: <UserExperience nav={nav} account={account} initialTab={current.data?.initialTab || "Preferences"} premium={premium} premiumLoading={premiumLoading} premiumError={premiumError} />, premiumProcessing: <PremiumProcessing nav={nav} data={current.data} />, profileCollections: <ProfileCollections nav={nav} account={account} initialTab={current.data?.initialTab || "Followed Artists"} />,
  ...
  return (
    <div className="ev-app-viewport flex min-h-screen w-full items-stretch justify-stretch overflow-hidden" style={{ background: C.bg, minHeight: "100dvh", width: "100dvw" }}>
      ...
      <AudioController ref={audioControllerRef} song={song} playing={playing} setPlaying={setPlaying} userId={account?.user?.id} onProgress={setPlayback} onEnded={advanceNext} />
...
function LegalDocumentScreen({ nav, documentType }) {
  ...
  useEffect(() => {
    let mounted = true;
    setLoading(true);
    loadPublishedLegalDocument(documentType).then((value) => mounted && setDocument(value)).catch((loadError) => mounted && setError(loadError.message || "Unable to load this legal document.")).finally(() => mounted && setLoading(false));
    return () => { mounted = false; };
  }, [documentType]);
  return <Phone><TopBack title={documentType === "PRIVACY_POLICY" ? "Privacy Policy" : "Terms & Conditions"} onBack={nav.pop} /><div className="flex-1 overflow-y-auto px-5 pb-8 pt-2">{loading ? <p className="py-10 text-center text-[13px]" style={{ color: C.muted }}>Loading the current published document...</p> : error ? <EmptyResourceCard label="Legal document unavailable" description={error} /> : !document ? <EmptyResourceCard label="Document not configured" description="This document has not been published yet. Please check back later." /> : <article><div className="mb-5"><p className="text-[10px] uppercase tracking-[0.18em]" style={{ color: C.goldSoft }}>{document.document_type === "PRIVACY_POLICY" ? "Privacy" : "Terms"} · Version {document.version}</p><h1 className="mt-2 text-2xl font-semibold" style={{ color: C.ivory }}>{document.title}</h1><p className="mt-3 text-[13px] leading-6" style={{ color: C.muted }}>{document.intro}</p><p className="mt-3 text-[10px]" style={{ color: C.muted }}>Effective {document.effective_date ? new Date(document.effective_date).toLocaleDateString("en-NG") : "date pending"} · Published {document.published_at ? new Date(document.published_at).toLocaleDateString("en-NG") : "date pending"}</p></div>{(Array.isArray(document.sections) ? document.sections : []).map((section, index) => <section key={section.id || `${document.id}-${index}`} className="mb-5"><h2 className="text-[15px] font-semibold" style={{ color: C.goldSoft }}>{section.heading || section.title || `Section ${index + 1}`}</h2><p className="mt-2 whitespace-pre-wrap text-[12px] leading-6" style={{ color: C.ivory }}>{section.body || section.content || ""}</p></section>)}{!document.sections?.length && <EmptyResourceCard label="No sections published" description="The Super Admin has not added any sections to this published document." />}</article>}</div></Phone>;
}

function AdminLegalCMS({ nav, account }) {
  const [documents, setDocuments] = useState([]);
  const [selected, setSelected] = useState(null);
  const [form, setForm] = useState({ documentType: "PRIVACY_POLICY", title: "", intro: "", version: "1.0", effectiveDate: "", sections: [] });
  const [busy, setBusy] = useState(false);
  const [message, setMessage] = useState("");
  const [error, setError] = useState("");
  const [preview, setPreview] = useState(false);
  const refresh = async () => { setError(""); try { setDocuments(await loadAdminLegalDocuments()); } catch (loadError) { setError(loadError.message || "Unable to load legal documents."); } };
  useEffect(() => { if (account?.user?.id) refresh(); }, [account?.user?.id]);
  const choose = (doc) => { setSelected(doc); setForm({ id: doc.id, documentType: doc.document_type, title: doc.title || "", intro: doc.intro || "", version: doc.version || "1.0", effectiveDate: doc.effective_date || "", sections: Array.isArray(doc.sections) ? doc.sections : [] }); setMessage(""); };
  const newDocument = () => { setSelected(null); setForm({ documentType: "PRIVACY_POLICY", title: "", intro: "", version: "1.0", effectiveDate: "", sections: [] }); setMessage(""); setError(""); };
  const save = async () => { setBusy(true); setMessage(""); setError(""); try { const saved = await saveAdminLegalDocument(form); setSelected(saved); setForm((current) => ({ ...current, id: saved.id })); setMessage("Draft saved."); await refresh(); } catch (saveError) { setError(saveError.message || "Unable to save draft."); } finally { setBusy(false); } };
  const publish = async () => { if (!form.id) return setError("Save the draft before publishing."); setBusy(true); setMessage(""); setError(""); try { await publishAdminLegalDocument(form.id); setMessage("Document published. The user-facing page now reads this version."); await refresh(); } catch (publishError) { setError(publishError.message || "Unable to publish document."); } finally { setBusy(false); } };
  const restore = async (doc) => { setBusy(true); setMessage(""); setError(""); try { const restored = await restoreAdminLegalDocument(doc.id); choose(restored); setMessage("A new draft was created from the selected version."); await refresh(); } catch (restoreError) { setError(restoreError.message || "Unable to restore version."); } finally { setBusy(false); } };
  const updateSection = (index, patch) => setForm((current) => ({ ...current, sections: current.sections.map((section, itemIndex) => itemIndex === index ? { ...section, ...patch } : section) }));
  const moveSection = (index, direction) => setForm((current) => { const next = [...current.sections]; const target = index + direction; if (target < 0 || target >= next.length) return current; [next[index], next[target]] = [next[target], next[index]]; return { ...current, sections: next }; });
  return <Phone><TopBack title="Legal / Policies" onBack={nav.pop} /><div className="flex-1 overflow-y-auto px-5 pb-8 pt-2">{message && <p className="mb-3 rounded-xl px-3 py-2 text-[12px]" style={{ background: C.green + "33", color: C.goldSoft }}>{message}</p>}{error && <p className="mb-3 rounded-xl px-3 py-2 text-[12px]" style={{ background: C.red + "22", color: C.red }}>{error}</p>}<div className="mb-4 flex gap-2"><GoldButton onClick={newDocument}>New document</GoldButton><GhostButton onClick={refresh}>Refresh</GhostButton></div><section className="mb-4 rounded-2xl p-4" style={{ background: C.card, border: `1px solid ${C.line}` }}><h2 className="mb-3 text-[14px] font-semibold" style={{ color: C.ivory }}>Version history</h2>{!documents.length ? <EmptyResourceCard label="No legal versions yet" description="Create the first Privacy Policy or Terms & Conditions draft." /> : documents.map((doc) => <div key={doc.id} className="flex items-center justify-between gap-2 border-b py-3" style={{ borderColor: C.line }}><button type="button" onClick={() => choose(doc)} className="min-w-0 flex-1 text-left"><p className="truncate text-[12px] font-semibold" style={{ color: C.ivory }}>{doc.document_type === "PRIVACY_POLICY" ? "Privacy Policy" : "Terms & Conditions"} · v{doc.version}</p><p className="mt-1 text-[10px] uppercase" style={{ color: doc.status === "PUBLISHED" ? C.goldSoft : C.muted }}>{doc.status} · {doc.updated_at ? new Date(doc.updated_at).toLocaleDateString("en-NG") : "date pending"}</p><p className="mt-1 text-[9px]" style={{ color: C.muted }}>Created by {doc.created_by || "unknown"} · Updated by {doc.updated_by || "unknown"}{doc.published_by ? ` · Published by ${doc.published_by}` : ""}</p></button>{doc.status === "PUBLISHED" && <button type="button" disabled={busy} onClick={async () => { setBusy(true); setError(""); try { await unpublishAdminLegalDocument(doc.id); setMessage("Document unpublished and archived."); await refresh(); } catch (unpublishError) { setError(unpublishError.message || "Unable to unpublish document."); } finally { setBusy(false); } }} className="rounded-xl px-2 py-2 text-[10px]" style={{ background: C.card2, color: C.ivory }}>Unpublish</button>}{doc.status !== "DRAFT" && <button type="button" disabled={busy} onClick={() => restore(doc)} className="rounded-xl px-2 py-2 text-[10px]" style={{ background: C.card2, color: C.ivory }}>Restore</button>}</div>)}</section><section className="rounded-2xl p-4" style={{ background: C.card, border: `1px solid ${C.line}` }}><h2 className="mb-3 text-[14px] font-semibold" style={{ color: C.ivory }}>{selected ? `Edit draft · ${form.documentType}` : "Document editor"}</h2><div className="grid grid-cols-2 gap-2"><select value={form.documentType} onChange={(event) => setForm((current) => ({ ...current, documentType: event.target.value }))} className="rounded-xl px-3 py-3 text-[12px]" style={{ background: C.card2, color: C.ivory, border: `1px solid ${C.line}` }}><option value="PRIVACY_POLICY">Privacy Policy</option><option value="TERMS_AND_CONDITIONS">Terms & Conditions</option></select><input value={form.version} onChange={(event) => setForm((current) => ({ ...current, version: event.target.value }))} placeholder="Version" className="rounded-xl px-3 py-3 text-[12px]" style={{ background: C.card2, color: C.ivory, border: `1px solid ${C.line}` }} /></div><input value={form.title} onChange={(event) => setForm((current) => ({ ...current, title: event.target.value }))} placeholder="Document title" className="mt-3 w-full rounded-xl px-3 py-3 text-[12px]" style={{ background: C.card2, color: C.ivory, border: `1px solid ${C.line}` }} /><textarea value={form.intro} onChange={(event) => setForm((current) => ({ ...current, intro: event.target.value }))} placeholder="Intro / subtitle" rows={3} className="mt-3 w-full rounded-xl px-3 py-3 text-[12px]" style={{ background: C.card2, color: C.ivory, border: `1px solid ${C.line}` }} /><input type="date" value={form.effectiveDate || ""} onChange={(event) => setForm((current) => ({ ...current, effectiveDate: event.target.value }))} className="mt-3 w-full rounded-xl px-3 py-3 text-[12px]" style={{ background: C.card2, color: C.ivory, border: `1px solid ${C.line}` }} />{form.sections.map((section, index) => <div key={section.id || index} className="mt-3 border-t pt-3" style={{ borderColor: C.line }}><div className="flex gap-2"><input value={section.heading || ""} onChange={(event) => updateSection(index, { heading: event.target.value })} placeholder={`Section ${index + 1} heading`} className="min-w-0 flex-1 rounded-xl px-3 py-3 text-[12px]" style={{ background: C.card2, color: C.ivory, border: `1px solid ${C.line}` }} /><button type="button" onClick={() => moveSection(index, -1)} className="rounded-xl px-2" style={{ background: C.card2, color: C.ivory }}>↑</button><button type="button" onClick={() => moveSection(index, 1)} className="rounded-xl px-2" style={{ background: C.card2, color: C.ivory }}>↓</button><button type="button" onClick={() => setForm((current) => ({ ...current, sections: current.sections.filter((_, itemIndex) => itemIndex !== index) }))} className="rounded-xl px-2" style={{ background: C.red + "22", color: C.red }}>×</button></div><textarea value={section.body || ""} onChange={(event) => updateSection(index, { body: event.target.value })} placeholder="Section content" rows={4} className="mt-2 w-full rounded-xl px-3 py-3 text-[12px]" style={{ background: C.card2, color: C.ivory, border: `1px solid ${C.line}` }} /></div>)}<div className="mt-3 flex gap-2"><GhostButton onClick={() => setForm((current) => ({ ...current, sections: [...current.sections, { heading: "", body: "" }] }))}>Add section</GhostButton><GhostButton onClick={() => setPreview((current) => !current)}>{preview ? "Close preview" : "Preview"}</GhostButton><GoldButton disabled={busy || selected?.status === "PUBLISHED"} onClick={save}>Save draft</GoldButton>{form.id && <GoldButton disabled={busy} onClick={publish}>Publish</GoldButton>}</div>{preview && <div className="mt-4 rounded-2xl p-4" style={{ background: C.bg, border: `1px solid ${C.line}` }}><p className="text-[10px] uppercase tracking-[0.16em]" style={{ color: C.goldSoft }}>Draft preview · v{form.version}</p><h3 className="mt-2 text-[20px] font-semibold" style={{ color: C.ivory }}>{form.title || "Untitled document"}</h3><p className="mt-2 whitespace-pre-wrap text-[12px] leading-5" style={{ color: C.muted }}>{form.intro}</p>{form.sections.map((section, index) => <div key={section.id || index} className="mt-4"><p className="text-[13px] font-semibold" style={{ color: C.goldSoft }}>{section.heading || `Section ${index + 1}`}</p><p className="mt-1 whitespace-pre-wrap text-[12px] leading-5" style={{ color: C.ivory }}>{section.body}</p></div>)}</div>}</section></div></Phone>;
}
