...
function OrganizerEvents({ nav, account }) {
  const [events, setEvents] = useState([]); const [selected, setSelected] = useState(null); const [dashboard, setDashboard] = useState(null); const [mode, setMode] = useState("list"); const [busy, setBusy] = useState(false); const [error, setError] = useState(""); const [message, setMessage] = useState("");
  const blank = { title: "", description: "", event_type: "", city: "", starts_at: "", ends_at: "", cover_url: "" }; const [form, setForm] = useState(blank); const [ticket, setTicket] = useState({ name: "", price: "", capacity: "", sales_start: "", sales_end: "", maximum_per_customer: "4" }); const [artistId, setArtistId] = useState("");
  ...
  const choose = async (event) => { setSelected(event); setForm({ title: event.title || "", description: event.description || "", event_type: event.event_type || "", city: event.city || "", starts_at: event.starts_at ? event.starts_at.slice(0, 16) : "", ends_at: event.ends_at ? event.ends_at.slice(0, 16) : "", cover_url: event.cover_url || "" }); setMode("detail"); try { setDashboard(await loadOrganizerEventDashboard(event.id)); } catch (loadError) { setError(loadError.message || "Unable to load event analytics."); } };
  const saveEvent = async (event) => { event.preventDefault(); setBusy(true); setError(""); try { const next = selected ? await updateOrganizerEvent(selected.id, account.user.id, form) : await createOrganizerEvent(account.user.id, form); setMessage(selected ? "Event draft saved." : "Draft event created."); await refresh(); setSelected(next); setMode("detail"); } catch (saveError) { setError(saveError.message || "Unable to save event."); } finally { setBusy(false); } };
