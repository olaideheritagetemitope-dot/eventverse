...
export default function CheckInScreen({ nav, data }) {
  ...
  const submit = async (event) => {
    ...
    try {
      const value = staffMode ? await eventStaffEntryDecision(qrToken.trim(), "ACCEPT") : await checkInTicketWithToken(qrToken.trim());
      setResult(value);
      setQrToken("");
    } catch (submitError) {
  ...
  return <div className="ev-root ev-app-viewport" style={{ background: C.bg, color: C.ivory }}><div className="mx-auto flex min-h-screen w-full max-w-[520px] flex-col"><div className="flex items-center gap-3 px-5 pb-4 pt-8" style={{ borderBottom: `1px solid ${C.line}` }}><button type="button" onClick={nav.pop} aria-label="Back" style={{ color: C.gold }}><span aria-hidden="true">←</span></button><div><p className="text-[11px] uppercase tracking-[0.16em]" style={{ color: C.gold }}>Protected operations</p><h1 className="ev-display text-[22px]" style={{ color: C.ivory }}>Ticket check-in</h1></div></div><main className="flex-1 px-5 py-6"><div className="rounded-2xl p-5" style={{ background: C.card, border: `1px solid ${C.line}` }}><ScanLine size={28} color={C.gold} /><p className="mt-4 text-[13px] leading-6" style={{ color: C.muted }}>{staffMode ? `${data.responsibility.replace("_", " ")} entry desk for ${data.eventTitle || "your assigned event"}.` : "Scan a verified Atizzy QR code or enter the server-issued QR token manually."} Authorization, ticket validity, and one-time check-in are enforced by Supabase.</p>{scanning && <div className="relative mt-4 overflow-hidden rounded-xl" style={{ border: `1px solid ${C.gold}` }}><video ref={videoRef} muted playsInline className="h-56 w-full object-cover" /><button type="button" onClick={stopScanner} aria-label="Stop scanner" className="absolute right-2 top-2 rounded-full p-2" style={{ background: C.bg, color: C.ivory }}><X size={16} /></button></div>}<button type="button" onClick={scanning ? stopScanner : startScanner} className="mt-4 flex w-full items-center justify-center gap-2 rounded-xl py-3 text-[13px] font-semibold" style={{ background: `${C.gold}20`, color: C.goldSoft, border: `1px solid ${C.gold}66` }}><Video size={16} />{scanning ? "Stop QR scanner" : "Scan QR code"}</button>{scanError && <p className="mt-3 rounded-xl p-3 text-[12px]" style={{ color: C.red, background: `${C.red}18` }}>{scanError}</p>}<form onSubmit={submit} className="mt-5"><label className="text-[12px]" style={{ color: C.goldSoft }}>QR token</label><input value={qrToken} onChange={(event) => setQrToken(event.target.value)} placeholder="Paste a server-issued QR token" className="mt-2 w-full rounded-xl px-3 py-3 text-[13px] outline-none" style={{ background: C.bg, color: C.ivory, border: `1px solid ${C.line}` }} /><button disabled={busy || !qrToken.trim()} className="mt-4 w-full rounded-xl py-3 text-[13px] font-semibold disabled:opacity-40" style={{ background: C.gold, color: C.bg }}>{busy ? "Checking..." : staffMode ? "Confirm valid entry" : "Check in ticket"}</button>{staffMode && <button type="button" disabled={busy || !qrToken.trim()} onClick={async () => { setBusy(true); setError(""); setResult(null); try { const value = await eventStaffEntryDecision(qrToken.trim(), "REJECT"); setResult(value); setQrToken(""); } catch (rejectError) { setError(rejectError.message || "Unable to record entry decision."); } finally { setBusy(false); } }} className="mt-2 w-full rounded-xl py-3 text-[13px] font-semibold disabled:opacity-40" style={{ background: `${C.red}18`, color: C.red, border: `1px solid ${C.red}55` }}>Reject entry</button>}</form>{error && <p className="mt-4 rounded-xl p-3 text-[12px]" style={{ color: C.red, background: `${C.red}18` }}>{error}</p>}{result && <div className="mt-4 rounded-xl p-4" style={{ color: result.decision === "REJECT" || result.status === "already_checked_in" ? C.red : C.goldSoft, background: `${result.decision === "REJECT" || result.status === "already_checked_in" ? C.red : C.gold}18` }}><CheckCircle2 size={20} color={result.decision === "REJECT" || result.status === "already_checked_in" ? C.red : C.gold} /><p className="mt-2 text-[13px] font-semibold">{result.decision === "REJECT" ? (result.reason === "ALREADY_USED" ? "Entry rejected: ticket already used" : result.message || "Entry rejected") : result.status === "already_checked_in" ? "Ticket was already checked in" : "Valid entry recorded"}</p><p className="mt-1 text-[11px]" style={{ color: C.muted }}>Ticket: {result.ticket_id || "Not identified"}</p></div>}</div></main></div></div>;
}
