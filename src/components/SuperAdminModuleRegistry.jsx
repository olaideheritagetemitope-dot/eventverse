...
const MODULES = [
  ...
  { id: "artists", label: "Artists", group: "Directories" },
  { id: "organizers", label: "Organizers", group: "Directories" },
  { id: "venues", label: "Venue Managers", group: "Directories" },
  { id: "staff", label: "Event Staff", group: "Directories" },
  { id: "admins", label: "Admins", group: "Directories" },
...
export default function SuperAdminModuleRegistry({ snapshot, events, C, money, onSuspend, onReview, onEventStatus, onOpenModule }) {
  ...
  const filteredUsers = active === "users" ? users : users.filter((user) => (user.roles || []).some((role) => ({ artists: "ARTIST", organizers: "ORGANIZER", venues: "VENUE_MANAGER", staff: "EVENT_STAFF", admins: "ADMIN" }[active] === role)));
  const open = (id) => { setActive(id); onOpenModule?.(id); };
  const title = MODULES.find((item) => item.id === active)?.label || "Overview";
  ...
  return <section className="rounded-2xl p-4 mb-4" style={{ background: C.card, border: `1px solid ${C.line}` }}>
    ...
    {["users", "artists", "organizers", "venues", "staff", "admins"].includes(active) && <div><div className="flex items-center justify-between mb-2"><p className="text-[12px] font-semibold" style={{ color: C.ivory }}>Live {title} directory</p><span className="text-[10px]" style={{ color: C.muted }}>{filteredUsers.length} records</span></div>{filteredUsers.slice(0, 50).map((user) => <div key={user.id} className="flex items-center justify-between gap-3 py-3 border-b" style={{ borderColor: C.line }}><div><p className="text-[12px]" style={{ color: C.ivory }}>{user.full_name || user.email || "Authenticated user"}</p><p className="text-[10px] mt-1" style={{ color: C.muted }}>{(user.roles || []).join(" · ") || "ATTENDEE"}</p></div><div className="flex gap-2"><button type="button" onClick={() => onSuspend?.(user.id, true)} className="text-[10px]" style={{ color: C.red }}>Block</button><button type="button" onClick={() => onSuspend?.(user.id, false)} className="text-[10px]" style={{ color: C.goldSoft }}>Restore</button></div></div>)}{!filteredUsers.length && <Empty label={`No ${title.toLowerCase()} records`} C={C} />}</div>}
