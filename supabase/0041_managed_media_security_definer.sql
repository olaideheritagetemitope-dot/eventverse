-- Fix managed-media validation permission errors without exposing internal helpers.
-- The helper is only called by the write trigger, never by the browser directly.
...
-- Trigger execution uses the table-writing role, so both trigger helpers must be
-- security definer. They remain private to the database; no browser role gets EXECUTE.
...
-- Reassert the existing managed-media triggers after replacing their functions.
