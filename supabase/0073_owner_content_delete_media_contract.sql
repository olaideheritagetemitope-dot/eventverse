-- Correct permanent-delete media cleanup to match the actual uploadMediaFile contract.
-- uploadMediaFile records entity_type as the caller-provided lowercase table name
-- ("songs" / "venues") and media_kind as MUSIC_COVER, AUDIO, MUSIC_VIDEO, or VENUE_PHOTO.
...
begin
  ...
  where ma.entity_id = p_song_id
    and lower(coalesce(ma.entity_type, '')) in ('song', 'songs', 'music', 'music_video')
    and ma.media_kind in ('MUSIC_COVER','AUDIO','MUSIC_VIDEO','SONG','MUSIC');

  delete from public.media_assets
  where entity_id = p_song_id
    and lower(coalesce(entity_type, '')) in ('song', 'songs', 'music', 'music_video')
    and media_kind in ('MUSIC_COVER','AUDIO','MUSIC_VIDEO','SONG','MUSIC');
