...
-- Root fix for the venue-posting failure:
-- the later artist-background migration reintroduced direct NEW field access into
-- a trigger shared by heterogeneous tables. A venue row has image_urls, but does
-- not have avatar_url/image_url/cover_url. Always inspect the row as JSONB.
...
begin
  ...
  if tg_table_name = 'venues' then
    ...
    if tg_table_name = 'artists'
      and not public.is_atizzy_managed_media_url(v_row ->> 'background_url') then
...
-- Reassert the table-specific trigger columns so this function is never invoked
-- with an unrelated media column list.
