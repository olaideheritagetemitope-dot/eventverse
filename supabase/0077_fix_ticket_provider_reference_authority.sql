...
-- 0077: make the ticket provider-reference handoff robust across PostgREST
-- service-role requests. Provider references remain server-only: authenticated
-- clients must not be able to call this RPC directly.
...
declare
  ...
  v_request_role text := coalesce(nullif(auth.role(), ''), nullif(current_setting('request.jwt.claim.role', true), ''));
begin
