-- Resolve pgcrypto explicitly from its installed schema during ticket issuance.
-- The prior function used unqualified digest(), which fails when pgcrypto is not on the function search_path.
