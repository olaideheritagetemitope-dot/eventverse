...
-- The user_roles table stores role_id, not a role text/code column. The prior
-- delete RPCs referenced ur.role and failed before authorization could run.
-- Recreate the same destructive contracts with the canonical roles join.
...
begin
  ...
  for update;
  if not found then raise exception 'Song access denied or song does not exist'; end if;
  ...
  where entity_id = p_song_id and entity_type in ('SONG','MUSIC','MUSIC_VIDEO');
...
begin
  ...
  where v.id = p_venue_id and (v.owner_id = auth.uid() or v_role_allowed)
  for update;
  if not found then raise exception 'Venue access denied or venue does not exist'; end if;

  select count(*) into v_booking_count from public.venue_bookings vb where vb.venue_id = p_venue_id;
  select coalesce(array_agg(ma.object_path) filter (where ma.object_path is not null), '{}'::text[])
    into v_paths
  from public.media_assets ma
  ...
  values (auth.uid(), 'DELETE', 'VENUE', p_venue_id,
    jsonb_build_object('name', v_venue.name, 'media_paths', to_jsonb(v_paths),
      'detached_booking_count', v_booking_count, 'history_preserved', true));
  return jsonb_build_object('id', p_venue_id, 'media_paths', to_jsonb(v_paths),
    'detached_booking_count', v_booking_count, 'history_preserved', true);
end;
$$;

revoke all on function public.delete_artist_song(uuid) from public;
