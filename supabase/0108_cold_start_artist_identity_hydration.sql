-- 0108: hydrate artist identity in cold-start discovery collections
-- The cold-start RPC is a valid fallback when ranked discovery is empty or unavailable.
-- Keep its payload contract identical to the ranked discovery RPC.
...
begin
  select jsonb_build_object(
    ...
    'latestMusicVideos', coalesce((select jsonb_agg(to_jsonb(x) order by x.published_at desc nulls last,x.created_at desc) from (
      select mv.id,mv.artist_id,mv.title,mv.description,mv.thumbnail_url,mv.video_url,mv.status,mv.published_at,mv.created_at,a.name as artist,a.image_url as artist_image
      from public.music_videos mv left join public.artists a on a.id=mv.artist_id
      where mv.status='PUBLISHED' and mv.video_url is not null order by mv.published_at desc nulls last,mv.created_at desc limit v_limit
    ) x),'[]'::jsonb),
    'allMusicVideos', coalesce((select jsonb_agg(to_jsonb(x) order by x.published_at desc nulls last,x.created_at desc) from (
      select mv.id,mv.artist_id,mv.title,mv.description,mv.thumbnail_url,mv.video_url,mv.status,mv.published_at,mv.created_at,a.name as artist,a.image_url as artist_image
      from public.music_videos mv left join public.artists a on a.id=mv.artist_id
      where mv.status='PUBLISHED' and mv.video_url is not null order by mv.published_at desc nulls last,mv.created_at desc limit v_limit offset v_offset
    ) x),'[]'::jsonb),
