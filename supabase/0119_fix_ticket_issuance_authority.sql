-- Root fix for the production error:
-- "Only the payment webhook service can verify payments"
--
-- PostgREST service-role requests can expose the role through auth.role()
-- while request.jwt.claim.role may be absent depending on gateway/runtime.
-- Keep issuance service-only, but recognize both authoritative role sources.
