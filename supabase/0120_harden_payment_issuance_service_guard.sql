-- Harden the payment issuance authority check for PostgREST gateway variants.
-- Service-role requests may expose the role through auth.role(), the legacy
-- request.jwt.claim.role setting, or the JSON request.jwt.claims setting.
-- No authenticated end user role is accepted.
...
begin
  ...
  request_role := coalesce(
    ...
    nullif(
      case when nullif(claims, '') is null then null else claims::jsonb ->> 'role' end,
      ''
    )
  );

  if coalesce(request_role, '') <> 'service_role' and session_user <> 'postgres' then
  ...
  loop
    for ticket_index in 1..item_row.quantity loop
      ...
      values (
        ...
        order_row.user_id,
        encode(extensions.digest(gen_random_uuid()::text || clock_timestamp()::text, 'sha256'::text), 'hex'),
        'ISSUED'
      );
    end loop;
